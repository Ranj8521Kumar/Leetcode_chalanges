...
-- Another Simplyfy and clear Approach using filter Query:
select s.user_id, round(coalesce(count(*) filter (where c.action = 'confirmed')*1.0 /nullif(count(c.user_id), 0), 0), 2) as confirmation_rate
from Signups s
