class Solution {
public:
    // Functio for clockWiseRotation:
    vector<vector<int>> clockWiseRotation(vector<vector<int>>& grid){
        int m = grid.size();
        int n = grid[0].size();
    ...
    // Function for compute three non-verlapping triangle with minArea:
    int computeTringlesWithMinArea(vector<vector<int>> &grid){
        int m = grid.size();
        int n = grid[0].size();
    ...
    int minimumSum(vector<vector<int>>& grid) {
        int result = INT_MAX;
        result = min(result, computeTringlesWithMinArea(grid));

        vector<vector<int>> rotatedGrid = clockWiseRotation(grid);

        result = min(result, computeTringlesWithMinArea(rotatedGrid));
